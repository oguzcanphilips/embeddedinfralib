...
public:
    ...
    {
        serverCertificates.AddCertificateAuthority(infra::BoundedConstString(mbedtls2_test_cas_pem, mbedtls2_test_cas_pem_len));
        serverCertificates.AddOwnCertificate(infra::BoundedConstString(mbedtls2_test_srv_crt, mbedtls2_test_srv_crt_len), infra::BoundedConstString(mbedtls2_test_srv_key, mbedtls2_test_srv_key_len));

        clientCertificates.AddCertificateAuthority(infra::BoundedConstString(mbedtls2_test_cas_pem, mbedtls2_test_cas_pem_len));
...
TEST_F(ConnectionMbedTlsTest, create_connection)
{
    ...
    EXPECT_CALL(serverObserverFactory, ConnectionAcceptedMock(testing::_, testing::_))
        .WillOnce(infra::Lambda([&](infra::AutoResetFunction<void(infra::SharedPtr<services::ConnectionObserver> connectionObserver)> createdObserver, services::IPv4Address ipv4Address)
    {
        createdObserver(observer1.Emplace());
    }));
    EXPECT_CALL(clientObserverFactory, ConnectionEstablishedMock(testing::_))
        .WillOnce(infra::Lambda([&](infra::AutoResetFunction<void(infra::SharedPtr<services::ConnectionObserver> connectionObserver)> createdObserver)
    {
