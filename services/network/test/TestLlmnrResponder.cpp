...
public:
    LlmnrResponderTest()
        ...
        , execute([this]()
            {
                EXPECT_CALL(factory, ListenIPv4(testing::Ref(responder), 5355, true)).WillOnce(testing::Return(datagramExchangePtr));
...
TEST_F(LlmnrResponderTest, request_record_a)
{
    ...
    infra::ByteInputStream requestStream(infra::MakeRange(data), infra::softFail);

    EXPECT_CALL(datagramExchange, RequestSendStream(38, services::UdpSocket{ services::Udpv4Socket{ { 1, 2, 3, 4 }, 12 } }));
    responder.DataReceived(requestStream, services::Udpv4Socket{ { 1, 2, 3, 4 }, 12 });

    EXPECT_CALL(ipv4Info, GetIPv4Address()).WillOnce(testing::Return(services::IPv4Address{ 5, 6, 7, 8 }));
    infra::ByteOutputStream::WithStorage<512> responseStream;
    responder.SendStreamAvailable(infra::UnOwnedSharedPtr(responseStream));
    EXPECT_EQ((std::array<uint8_t, 38>{ 1, 2, 0x80, 0, 0, 1, 0, 1, 0, 0, 0, 0, 4, 'n', 'a', 'm', 'e', 0, 0, 1, 0, 1,
        0xc0, 0x0c, 0, 1, 0, 1, 0, 0, 0, 30, 0, 4, 5, 6, 7, 8 }), responseStream.Writer().Processed());
}
