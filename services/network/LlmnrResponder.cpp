...
namespace services
{
    ...
    LlmnrResponder::LlmnrResponder(services::DatagramFactory& factory, services::Multicast& multicast, services::IPv4Info& ipv4Info, infra::BoundedConstString name)
        : datagramExchange(factory.ListenIPv4(*this, llmnpPort, true))
        , multicast(multicast)
        , ipv4Info(ipv4Info)
        , name(name)
    {
    ...
    void LlmnrResponder::DataReceived(infra::DataInputStream stream, services::UdpSocket from)
    {
        if (replying)
            return;
    ...
    void LlmnrResponder::SendStreamAvailable(infra::SharedPtr<infra::DataOutputStream>&& stream)
    {
        replying = false;
