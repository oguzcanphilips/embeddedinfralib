...
#include "packs/upgrade/boot_loader/VerifierRsa.hpp"
...
namespace application
{
    ...
    bool VerifierRsa::IsValid(hal::SynchronousFlash& flash, const hal::SynchronousFlash::Range& signature, const hal::SynchronousFlash::Range& data) const
    {
        ...
        while (message != data.second)
        {
            ...
            mbedtls2_sha256_update(&ctx, buffer.data(), size);
            message += size;
        }

        mbedtls2_sha256_finish(&ctx, messageHash.data());                                                    //TICS !INT#030
        ...
        rsaCtx.N.p = reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(publicKeyN.begin()));
        rsaCtx.E.s = 1;
        rsaCtx.E.n = publicKeyE.size();
        rsaCtx.E.p = reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(publicKeyE.begin()));
        mbedtls2_rsa_set_padding(&rsaCtx, MBEDTLS_RSA_PKCS_V21, MBEDTLS_MD_SHA256);

        int ret = mbedtls2_rsa_rsassa_pss_verify(&rsaCtx, nullptr, nullptr, MBEDTLS_RSA_PUBLIC, MBEDTLS_MD_SHA256, messageHash.size(), messageHash.data(), theSignature.data());
