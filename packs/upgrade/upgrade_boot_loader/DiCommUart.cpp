...
#include "upgrade_boot_loader/DiCommUart.hpp"
...
namespace application
{
    ...
    bool DiCommUart::Initialize()
    {
        ...
        CrcCcittCalculator crcCalculator;
    ...
    static const uint16_t crcCcittTabe[256] = {
    ...
    void DiCommUart::CrcCcittCalculator::Update(infra::ConstByteRange range)
    {
        for (uint8_t byte : range)
            crc = (crc << 8) ^ crcCcittTabe[(crc >> 8) ^ byte];
    }

    uint16_t DiCommUart::CrcCcittCalculator::Result() const
    {
        return crc;
    }
}
