...
namespace application
{
    ...
    bool ImageSignerRsa::CheckSignature(const std::vector<uint8_t>& signature, const std::vector<uint8_t>& image)
    {
        ...
        rsaCtx.N.p = reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(publicKey.N.begin()));
        rsaCtx.E.s = 1;
    ...
    void ImageSignerRsa::CalculateSignature(const std::vector<uint8_t>& image)
    {
        ...
        rsaCtx.D.p = reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(privateKey.D.begin()));
        rsaCtx.P.s = 1;
        rsaCtx.P.n = privateKey.P.size() / 4;
        rsaCtx.P.p = reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(privateKey.P.begin()));
        rsaCtx.Q.s = 1;
        rsaCtx.Q.n = privateKey.Q.size() / 4;
        rsaCtx.Q.p = reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(privateKey.Q.begin()));
        rsaCtx.DP.s = 1;
        rsaCtx.DP.n = privateKey.DP.size() / 4;
        rsaCtx.DP.p = reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(privateKey.DP.begin()));
        rsaCtx.DQ.s = 1;
        rsaCtx.DQ.n = privateKey.DQ.size() / 4;
        rsaCtx.DQ.p = reinterpret_cast<uint32_t*>(const_cast<uint8_t*>(privateKey.DQ.begin()));
        rsaCtx.QP.s = 1;
