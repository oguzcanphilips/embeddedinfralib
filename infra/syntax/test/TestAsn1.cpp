...
TEST(Asn1Test, dereference_iterator)
{
    ...
    EXPECT_EQ(infra::ConstByteRange(data.data() + 2, data.data() + 3), (*const_cast<const infra::Asn1SequenceIterator&>(sequence.begin())).Integer());
    EXPECT_EQ(infra::ConstByteRange(data.data() + 2, data.data() + 3), sequence.begin().operator->()->Integer());
    EXPECT_EQ(infra::ConstByteRange(data.data() + 2, data.data() + 3), const_cast<const infra::Asn1SequenceIterator&>(sequence.begin()).operator->()->Integer());
}
