...
typedef struct
{
    mbedtls2_ecp_group grp;      /*!<  elliptic curve used                           */
    mbedtls2_mpi d;              /*!<  our secret value (private key)                */
    mbedtls2_ecp_point Q;        /*!<  our public value (public key)                 */
    mbedtls2_ecp_point Qp;       /*!<  peer's public value (public key)              */
    mbedtls2_mpi z;              /*!<  shared secret                                 */
    int point_format;   /*!<  format for point export in TLS messages       */
    mbedtls2_ecp_point Vi;       /*!<  blinding value (for later)                    */
    mbedtls2_ecp_point Vf;       /*!<  un-blinding value (for later)                 */
    mbedtls2_mpi _d;             /*!<  previous d (for later)                        */
}
 ...
 *                  attacks, see \c mbedtls2_ecp_mul() for details.
 */
 ...
 *                  properly set (for example using mbedtls2_ecp_group_load).
 *
 ...
 * \param f_rng     RNG function, see notes for \c mbedtls2_ecdh_compute_shared()
 * \param p_rng     RNG parameter
 *
 * \return          0 if successful, or an MBEDTLS_ERR_ECP_XXX error code
 */
